...
export default function AdminTestimonials() {
  ...
  const save = useMutation({
    mutationFn: (data: Partial<Testimonial>) => {
      ...
      return editing === 'new'
        ? adminApiService.createTestimonial(key, data)
        : adminApiService.updateTestimonial(key, (editing as Testimonial).id, data)
    },
    onSuccess: () => { 
      toast.success('Salvo!')
  ...
  const del = useMutation({
    // CORREÇÃO: Injeta a chave admin e o ID correspondente do depoimento
    mutationFn: (id: number) => adminApiService.deleteTestimonial(getAdminKey(), id),
    onSuccess: () => { 
          ...
          <div className="card-glass w-full max-w-lg p-8">
            ...
            <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-4">
              ...
              <div className="flex items-center gap-6 pt-1">
                ...
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="actv" {...register('active')} className="w-4 h-4 accent-brand-500 rounded border-white/10 bg-white/5" />
                  <label htmlFor="actv" className="text-sm text-slate-300 font-medium select-none">Registro Ativo?</label>
                </div>
              </div>
